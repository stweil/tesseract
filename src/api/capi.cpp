...
TESS_API const char* TESS_CALL TessVersion() { return TessBaseAPI::Version(); }

TESS_API void TESS_CALL TessDeleteText(char* text) { delete[] text; }

TESS_API void TESS_CALL TessDeleteTextArray(char** arr) {
  for (char** pos = arr; *pos != nullptr; ++pos) delete[] * pos;
  delete[] arr;
}

TESS_API void TESS_CALL TessDeleteIntArray(int* arr) { delete[] arr; }
...
TESS_API TessBaseAPI* TESS_CALL TessBaseAPICreate() { return new TessBaseAPI; }
...
TESS_API int TESS_CALL TessBaseAPIInit4(
  ...
  return handle->Init(datapath, language, mode, configs, configs_size,
                      &varNames, &varValues, set_only_non_debug_params);
}
...
TESS_API struct Boxa* TESS_CALL TessBaseAPIGetComponentImages1(
    ...
    struct Pixa** pixa, int** blockids, int** paraids) {
  return handle->GetComponentImages(level, text_only != FALSE, raw_image,
...
TESS_API BOOL TESS_CALL TessBaseAPIProcessPage(TessBaseAPI* handle,
                                               ...
                                               int timeout_millisec,
                                               TessResultRenderer* renderer) {
  if (handle->ProcessPage(pix, page_index, filename, retry_config,
...
TESS_API void TESS_CALL TessBaseAPIEnd(TessBaseAPI* handle) { handle->End(); }
...
TESS_API void TESS_CALL TessPageIteratorParagraphInfo(
  ...
  handle->ParagraphInfo(justification, &bool_is_list_item, &bool_is_crown,
                        first_line_indent);
  if (is_list_item) *is_list_item = bool_is_list_item ? TRUE : FALSE;
  if (is_crown) *is_crown = bool_is_crown ? TRUE : FALSE;
}
...
TESS_API const char* TESS_CALL TessResultIteratorWordFontAttributes(
  ...
  bool bool_is_bold, bool_is_italic, bool_is_underlined, bool_is_monospace,
      bool_is_serif, bool_is_smallcaps;
  ...
  if (is_bold) *is_bold = bool_is_bold ? TRUE : FALSE;
  if (is_italic) *is_italic = bool_is_italic ? TRUE : FALSE;
  if (is_underlined) *is_underlined = bool_is_underlined ? TRUE : FALSE;
  if (is_monospace) *is_monospace = bool_is_monospace ? TRUE : FALSE;
  if (is_serif) *is_serif = bool_is_serif ? TRUE : FALSE;
  if (is_smallcaps) *is_smallcaps = bool_is_smallcaps ? TRUE : FALSE;
  return ret;
}
...
TESS_API ETEXT_DESC* TESS_CALL TessMonitorCreate() { return new ETEXT_DESC(); }
