...
static void AffineMatrix(int writing_direction, int line_x1, int line_y1,
                         int line_x2, int line_y2, double* a, double* b,
                         double* c, double* d) {
...
static void ClipBaseline(int ppi, int x1, int y1, int x2, int y2, int* line_x1,
  ...
  *line_y2 = y2;
  ...
  if (rise < 2 * ppi && 2 * ppi < run) *line_y1 = *line_y2 = (y1 + y2) / 2;
}
...
bool TessPDFRenderer::imageToPDFObj(Pix* pix, const char* filename,
                                    long int objnum, char** pdf_object,
  ...
  if (!pdf_object_size || !pdf_object) return false;
  *pdf_object = nullptr;
  *pdf_object_size = 0;
  if (!filename && !pix) return false;
...
bool TessPDFRenderer::EndDocumentHandler() {
  ...
  n = snprintf(buf, sizeof(buf),
               ...
               ">>\n"
               "endobj\n",
               obj_, tesseract::TessBaseAPI::Version(), datestr,
               utf16_title.c_str());
  lept_free(datestr);
  if (n >= sizeof(buf)) return false;
  AppendPDFObject(buf);
