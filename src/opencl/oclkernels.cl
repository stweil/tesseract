...
__kernel void composeRGBPixel(__global uint *tiffdata, int w, int h, int wpl,
  ...
  output[i * wpl + j] = (rval << (8 * (sizeof(uint) - 1 - 0))) |
                        (gval << (8 * (sizeof(uint) - 1 - 1))) |
                        (bval << (8 * (sizeof(uint) - 1 - 2)));
}
...
__kernel void morphoDilateHor(__global int *sword, __global int *dword,
                              const int xp, const int xn, const int wpl,
                              const int h) {
  const int col = get_global_id(0);
  const int row = get_global_id(1);
  const unsigned int pos = row * wpl + col;
  ...
  unsigned int lnextword, lprevword, rnextword, rprevword, firstword,
      secondword;
  int i, j, siter, eiter;
  ...
  for (i = 1; i <= parbitsxp; i++) {
    // Get the max value on LHS of every pixel
    tempword = ((i == parbitsxp) && (parbitsxp != parbitsxn))
                   ? 0x0
                   : (firstword << (32 - i)) | ((currword >> i));
...
__kernel void morphoDilateVer(__global int *sword, __global int *dword,
                              const int yp, const int wpl, const int h,
                              const int yn) {
  const int col = get_global_id(0);
  const int row = get_global_id(1);
  const unsigned int pos = row * wpl + col;
...
__kernel void morphoErodeVer_5x5(__global int *sword, __global int *dword,
                                 const int wpl, const int h, const int fwmask,
                                 const int lwmask) {
  const int col = get_global_id(0);
  const int row = get_global_id(1);
  const unsigned int pos = row * wpl + col;
...
__kernel void morphoErodeHor(__global int *sword, __global int *dword,
                             const int xp, const int xn, const int wpl,
                             const int h, const char isAsymmetric,
...
kernel_HistogramRectAllChannels(__global const uchar8 *data, uint numPixels,
  ...
  for (uint pc = get_global_id(0);
       pc < numPixels * NUM_CHANNELS / HR_UNROLL_SIZE;
       pc += get_global_size(0)) {
    pixels = data[pc];
    //                       channel                        bin thread
    atomic_inc(&histBuffer[0 * HIST_SIZE * HIST_REDUNDANCY +
                           pixels.s0 * HIST_REDUNDANCY + threadOffset]); // ch0
    atomic_inc(&histBuffer[0 * HIST_SIZE * HIST_REDUNDANCY +
                           pixels.s4 * HIST_REDUNDANCY + threadOffset]); // ch0
    atomic_inc(&histBuffer[1 * HIST_SIZE * HIST_REDUNDANCY +
                           pixels.s1 * HIST_REDUNDANCY + threadOffset]); // ch1
    atomic_inc(&histBuffer[1 * HIST_SIZE * HIST_REDUNDANCY +
                           pixels.s5 * HIST_REDUNDANCY + threadOffset]); // ch1
    atomic_inc(&histBuffer[2 * HIST_SIZE * HIST_REDUNDANCY +
                           pixels.s2 * HIST_REDUNDANCY + threadOffset]); // ch2
    atomic_inc(&histBuffer[2 * HIST_SIZE * HIST_REDUNDANCY +
                           pixels.s6 * HIST_REDUNDANCY + threadOffset]); // ch2
    atomic_inc(&histBuffer[3 * HIST_SIZE * HIST_REDUNDANCY +
                           pixels.s3 * HIST_REDUNDANCY + threadOffset]); // ch3
    atomic_inc(&histBuffer[3 * HIST_SIZE * HIST_REDUNDANCY +
                           pixels.s7 * HIST_REDUNDANCY + threadOffset]); // ch3
  }
}

// NUM_CHANNELS = 1
__attribute__((reqd_work_group_size(256, 1, 1))) __kernel void
kernel_HistogramRectOneChannel(__global const uchar8 *data, uint numPixels,
                               __global uint *histBuffer) {
  ...
  // for each pixel/channel, accumulate in global memory
  for (uint pc = get_global_id(0); pc < numPixels / HR_UNROLL_SIZE;
       pc += get_global_size(0)) {
    pixels = data[pc];
    //                        bin                         thread
...
__attribute__((reqd_work_group_size(256, 1, 1))) __kernel void
kernel_HistogramRectAllChannelsReduction(int n, // unused pixel redundancy
                                         __global uint *histBuffer,
  ...
  for (uint i = get_local_id(0); i < HIST_REDUNDANCY; i += GROUP_SIZE) {
    value += histBuffer[channel * HIST_SIZE * HIST_REDUNDANCY +
                        bin * HIST_REDUNDANCY + i];
  }
...
// NUM_CHANNELS = 1
__attribute__((reqd_work_group_size(256, 1, 1))) __kernel void
kernel_HistogramRectOneChannelReduction(int n, // unused pixel redundancy
                                        __global uint *histBuffer,
                                        __global int *histResult) {
...
__attribute__((reqd_work_group_size(256, 1, 1))) __kernel void
kernel_ThresholdRectToPix(__global const uchar4 *imageData, int height,
                          int width,
                          int wpl, // words per line
                          __global int *thresholds, __global int *hi_values,
                          __global int *pix) {
    ...
    for (int b = 0; b < BURSTS_PER_WORD; b++) {
      ...
      for (int p = 0; p < PIXELS_PER_BURST; p++) {
        for (int c = 0; c < NUM_CHANNELS; c++) {
          ...
          if (pHi_Values[c] >= 0 &&
              (pixChan > pThresholds[c]) == (pHi_Values[c] == 0)) {
...
__attribute__((reqd_work_group_size(256, 1, 1))) __kernel void
kernel_ThresholdRectToPix_OneChan(__global const uchar8 *imageData, int height,
                                  int width,
                                  int wpl, // words per line of output image
                                  __global int *thresholds,
                                  __global int *hi_values, __global int *pix) {
  ...
  for (uint w = get_global_id(0); w < wpl * height; w += get_global_size(0)) {
    ...
    for (int b = 0; b < BURSTS_PER_WORD; b++) {
      ...
      for (int p = 0; p < PIXELS_PER_BURST; p++) {
        ...
        if (pHi_Values[0] >= 0 &&
            (pixChan > pThresholds[0]) == (pHi_Values[0] == 0)) {
          const uint kTopBit = 0x80000000;
          word |= (kTopBit >> ((b * PIXELS_PER_BURST + p) & 31));
        }
      }
