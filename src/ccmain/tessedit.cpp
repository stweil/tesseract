...
void Tesseract::read_config_file(const char *filename,
  ...
  tesserr << "read_config_file" << std::endl;
  ...
  ParamUtils::ReadParamsFile(selected_path.c_str(), constraint,
                             this->params());
}
...
bool Tesseract::init_tesseract_lang_data(const std::string &arg0,
  ...
  tesserr << "init_tesseract_lang_data, tessdata_path=" << tessdata_path.c_str() << std::endl;
  tesserr << "init_tesseract_lang_data, tessdata_path=" << tessdata_path.string().c_str() << std::endl;
